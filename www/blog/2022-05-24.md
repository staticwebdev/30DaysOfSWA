...
Yesterday we talked about end-to-end developer experience. Today we'll explore adding search to a website.
 ...
 * Deploy to SWA
 * **Exercise**: Upload data to search index. Then deploy sample to SWA.

![](../static/img/series/banner.png)

Today we explore adding search to a website with Azure Cognitive Search. Azure Cognitive Search provides many capabilities for searching and even provides extensibility with provided or custom functionality.

The [complete source code](https://github.com/Azure-Samples/azure-search-javascript-samples/blob/master/search-website/) is provided for you.

![Sample with Cognitive Search functionality](../static/img/series/w4d2/cognitive-search-enabled-book-website.png)

This example of Cognitive Search shows the most common use of search for a website and demonstrates how fast it is to get up and running. This sample website provides access to a catalog of 10,000 books. A user can search the catalog by entering text in the search bar. While the user enters text, the website uses the Search Index's suggest feature to complete the text. Once the query finishes, the list of books is displayed with a portion of the details. A user can select a book to see all the details, stored in the Search Index, of the book.
...
Once you create your Azure resource for Cognitive Search, you can access the service with the following vital information:

* Resource name: what you named your Cognitive Search instance.
* Index name: your resource can hold several indexes, each identified with a name.
* Admin key: used to upload data into the search index
* Query key: used by the API to find data in the search index

## What is a search index?

Search functionality needs data to query. In Azure Cognitive Search, this data is held in a _search index_. Because this sample is already written for you, that includes the upload of the JSON data into the search index. 

The trickiest part of this process is mapping your data into Cognitive Search types. In order to do this, you provide a [schema file](https://github.com/Azure-Samples/azure-search-javascript-samples/blob/master/search-website/bulk-insert/good-books-index.json) that describes each data field. 
