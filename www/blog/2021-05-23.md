...
I like structure, repeatability, patterns & conventions. I appreciate having a framework, an opinion many agree with so I'm on the same page with my team. And I like all of that, because it oftentimes leads to quick results and success.

## Experiences are like roads

Experiences can take you places .. or not. To me, that means **development experiences are a lot like the roads we travel on**:

 * They have milestones, to measure progress.
 * They have multiple lanes, for options.
 * They have entry and exit ramps, for flexibility. 
 * And they should get you somewhere (destination).
...
Now we're speeding on a multilane freeway with many twists and turns, interconnecting roads, and numerous possiblities ([aka configurations](https://dev.to/azure/04-configuring-static-web-apps-2oa7)). To get to your destination, you may have to use ramps to connect two different roads -- similar to how we connect different services [via an API](https://dev.to/azure/03-api-enabling-static-web-apps-25kh). 
...
- insufficient signposts, or appearing close enough to a detour that don't have enough time to follow instructions
...
If you're driver, and a developer reading this list, you probably can guess what each one of those hypothetical ingredients to a bad riding experience, map to. 
...
**Because, let's face it: developing should be a fun experience.** 

And for that you need the right vehicle, the right infrastructure, and a plan. _And when you lack all that, it's like riding a Lamborghini across a sandy desert._
