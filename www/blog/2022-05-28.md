...
title: "#27: Build a video conferencing web app with Azure Communication Services"
authors: [davrous]
...
## What We'll Cover
 * What is Azure Communications Services aka ACS?
...
To give you a brief idea our starting point, have a look to this 2 min video tutorial. It will show you how to get the internal ACS auth token to connect to its infrastructure. Then, I’ll join a Teams meeting using a simple web page and the ACS JavaScript SDK:
...
If you’d like to follow it at home, you simply need to:
...
You’ll discover that, by default, Azure Communication Services is providing a basic authentication & identity layer. You need to build on top of it to connect your own identity platform. Let’s see how to do that!
...
We’re going to use it as our front end to welcome and authenticate our users. Then, we will map their social account identity with the internal ACS identity via an Azure Function. The Azure Function will store the mapping into a small CosmosDB table. 

Here’s the flow. First, you’ll land on a page asking you to choose your favorite social provider:
...
In the same sample, I’ve been building on top of the architecture described just above a small metaverse using [Babylon.js](https://www.babylonjs.com), an open source & free WebGL engine I’ve been contributing to. The logic is identical to what I’ve described up to now. Thanks to the SWA, I’m authenticating the user using his social profile. But then, I can do the video call to another ACS user or someone in Teams from a 3D scene. And everything is waayyy cooler inside a 3D scene, right? 

![Screenshot of the Static Web App with a 3D scene of a museum loaded](../static/img/series/w4d6/ACSSWA004.jpg)

But wait, there’s even better. You can experience this Babylon.js in VR thanks to its native WebXR support! To give you an idea, here’s a video where I’m using my Valve Index to call my girl friend in Teams so we can visit this museum together.
